...
#    dataframe['content'] = dataframe['content'].replace(r'\bFrom(.*?)\nSubject(.*?)Re(.*?)\nOrganization(.*?)\nLine(.*?)\n\nRe(.*?)\n\n\b', '',regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'From(.*?)\n', ' ', regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'Subject(.*?)\n', ' ', regex=True)

    dataframe['content'] = dataframe['content'].replace(r'Line(.*?)\n', ' ', regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'Date(.*?)\n', ' ', regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'Organization(.*?)\n', ' ', regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'(NNTP-Posting-Host|Nntp-Posting-Host)(.*?)\n', ' ', regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'Distribution(.*?)\n', ' ', regex=True)
    
    # Remove end of line characters
    dataframe['content'] = dataframe['content'].replace(r'[\r\n]+', ' ', regex=True)

#    dataframe['content'] = dataframe['content'].replace(r'(From:|Organization)(.*?)(says:|writes:)', '', regex=True)
    
    dataframe['content'] = dataframe['content'].replace(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', '', regex=True)

    # dataframe['content'] = dataframe['content'].replace(r'\s+', '', regex=True)

    dataframe['content'] = dataframe['content'].replace(r'\_+', '', regex=True)
